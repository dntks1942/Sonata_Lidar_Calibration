		...
		VPointCloud &pc)
{
	...
	for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
		...
		//*UPPER_BANK - 0Xeeff (in src)
		//*UPPER_BANK - 0Xffee (in datasheet)
		...
		//* when value of block is 11, it uese previous azimuth_diff value since block 11 is the last one.
		...
		for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
			for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
				...
				/** Position Calculation */
				union two_bytes tmp; //* 2 set of 16 laser firing data in 1 block
				tmp.bytes[0] = raw->blocks[block].data[k];
				tmp.bytes[1] = raw->blocks[block].data[k+1];

				//* dsr ==> 16개의 laser중 몇 번째인가
				...
				if ((azimuth_corrected >= config_.min_angle
					...
					float distance_corr_x = 0;
					float distance_corr_y = 0;
					if (corrections.two_pt_correction_available) { //*VLP16db.yaml dosen't have two_pt_correction_available value
						distance_corr_x =
								(corrections.dist_correction - corrections.dist_correction_x)
								* (xx - 2.4) / (25.04 - 2.4)
								+ corrections.dist_correction_x;
					 ...
					 * model we used.
					 */
					xy_distance = distance_x * cos_vert_angle - vert_offset * sin_vert_angle ; //* formula as one in above
					x = xy_distance * sin_rot_angle - horiz_offset * cos_rot_angle;
