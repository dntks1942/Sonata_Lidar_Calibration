...
Transform::processScan(const velodyne_msgs::VelodyneScan::ConstPtr &scanMsg)
{
	...
	//ROS_DEBUG_STREAM("Publishing " << outMsg->height * outMsg->width	<< " Velodyne points, time: " << outMsg->header.stamp);
	//ROS_INFO("Publishing " << outMsg->height * outMsg->width << " Velodyne points, time: " << outMsg->header.stamp);
